...
namespace TopicDetail.Infrastructure.Repositories
{
    public class TopicDetailRepository : ITopicDetailRepository
    {
        ...
        {
            var answer = await _context.Answers.FindAsync(id); // Optimize: dùng FindAsync thay vì GetAnswerByIdAsync (không cần include cho delete)
            if (answer != null)
            {
        ...
        {
            return await _context.Votes.FindAsync(id); // Sửa: dùng FindAsync cho primary key, nhanh hơn FirstOrDefaultAsync
        }
        ...
        {
            var vote = await _context.Votes.FindAsync(id); // Optimize: dùng FindAsync trực tiếp, không cần GetVoteByIdAsync
            if (vote != null)
            {
