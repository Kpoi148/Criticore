...
namespace TopicDetail.Infrastructure.Models;

public partial class TopicDetailDbContext : DbContext
{
    public TopicDetailDbContext()
    {
    }

    public TopicDetailDbContext(DbContextOptions<TopicDetailDbContext> options)
        : base(options)
    {
    }
...
//        => optionsBuilder.UseSqlServer("Data Source=MSI;Initial Catalog=CriticoreDB;Persist Security Info=True;User ID=chikhanh;Password=khanh04;Encrypt=True;Trust Server Certificate=True");
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        {
            entity.HasKey(e => e.JoinRequestId).HasName("PK__JoinRequ__2573934AF5B05B55");
            ...
            entity.HasOne(d => d.Class).WithMany(p => p.JoinRequests)
                .HasForeignKey(d => d.ClassId)
                .OnDelete(DeleteBehavior.ClientSetNull)
                .HasConstraintName("FK__JoinReque__Class__3E1D39E1");

            entity.HasOne(d => d.ReviewedByNavigation).WithMany(p => p.JoinRequestReviewedByNavigations)
                .HasForeignKey(d => d.ReviewedBy)
                .HasConstraintName("FK__JoinReque__Revie__40058253");

            entity.HasOne(d => d.User).WithMany(p => p.JoinRequestUsers)
                .HasForeignKey(d => d.UserId)
                .OnDelete(DeleteBehavior.ClientSetNull)
                .HasConstraintName("FK__JoinReque__UserI__3F115E1A");
        });
